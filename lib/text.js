...
/**
 * Generate a short, random string based off the date. Note that the length isn’t fixed.
 *
...
export function pingablify(text) {
	...
	const pingable =
		...
		[...new Intl.Segmenter().segment("text")].length / 2;

	return pingable && text.length > 5
		? text
		: text.replaceAll(getRegex(), "") || "[pingable name]";
}
