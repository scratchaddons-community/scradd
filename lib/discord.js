 ...
 * @param {string} title
 * @param {<Fetch extends boolean | undefined>(
 * 	options: import("discord.js").WebhookEditMessageOptions & ({ fetchReply: Fetch } | {}),
 * ) => Promise<Fetch extends true ? import("discord.js").Message : void>} reply
 */
...
export async function paginate(array, toString, failMessage, title, reply) {
	...
	collector
		...
		.on("end", async () => {
			await reply({ components: message ? disableComponents(message.components) : [] });
		});
}
