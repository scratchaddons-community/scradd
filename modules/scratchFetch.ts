...
defineEvent("messageCreate", async (message) => {
	...
	switch (type) {
		case "projects":
			...
			const projectdata = await fetchApiData(`https://${constants.urls.scratchApi}/projects/${urlParts[4]}/`).catch(() => {});
			...
			break;
		case "users":
			const userdata = await fetchApiData(`https://${constants.urls.scratchApi}/users/${urlParts[4]}/`).catch(() => {});
			...
			break;
		case "studios":
			const studiodata = await fetchApiData(`https://${constants.urls.scratchApi}/studios/${urlParts[4]}/`).catch(() => {});
			message.reply({
						embeds: [
							{
