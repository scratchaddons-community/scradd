...
export default async (client) => {
	...
	/** @type {Collection<string, Omit<import("@discordjs/builders").SlashCommandBuilder, "addSubcommand" | "addSubcommandGroup">>} */
	const slashes = new Collection();
	commands.forEach((command, key) => slashes.set(key, command.data))
	await Promise.all(prexistingCommands.map(command => {
		if(slashes.has(command.name)) return
		return command.delete()
	}))

	await Promise.all(slashes.map(command => {
		if(prexistingCommands.has(command.name)) client.application?.commands.edit(command.name, command.toJSON(),
